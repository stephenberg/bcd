...
# test=NULL
# # microbenchmark(test<-multiResponseGaussianSparse(nGroups_ = nGroups,groupStart_ = groupStart
# #                                                 ,groupEnd_ = groupEnd,groupColumns_ = groupColumns
# #                                                 ,tol=10^-9,penaltyFactor_ = penaltyFactor
# #                                                 ,response=response,X_=XSparse,boundConstant_ = 1,
# #                                                 k_=1,nLambda_ = nLambda,maxit=1),times=1)
# # microbenchmark(test<-multiResponseGaussianDense(nGroups_ = nGroups,groupStart_ = groupStart
# #                                                  ,groupEnd_ = groupEnd,groupColumns_ = groupColumns
# #                                                  ,tol=10^-9,penaltyFactor_ = penaltyFactor
# #                                                  ,response=response,X_=X,boundConstant_ = 1,
# #                                                  k_=1,nLambda_ = nLambda,maxit=10000),times=1)
# 
# testDense=NULL
# testSparse=NULL
# lambdaMinRatio=grpregFit$lambda[nLambda]/grpregFit$lambda[1]
# microbenchmark(testDense<-bcdFit(X=X,y=response,penaltyFactor=penaltyFactor,groups=groups,tolerance = 10^-12,lambdaMinRatio=lambdaMinRatio,nLambda = nLambda),times=1)
...
# myBeta=betaMatFromList(testDense,nLambda)
