...
# grpregFit=NULL
# microbenchmark(grpregFit<-grpreg(X = X[,2:dim(X)[2]],y=z,eps = 10^-15,group=grouping,family="binomial",lambda.min = 0.05),times=1)
...
# 
# 
# lambda=testDense$lambda
# folds=caret::createFolds((1:dim(X)[1]))
# deviances=multinomialCV(X=X,response=response,nFolds = 10,folds=folds,k=2,groups = groups,lambda=lambda,penaltyFactor = penaltyFactor)
# plot(apply(deviances,1,sum))